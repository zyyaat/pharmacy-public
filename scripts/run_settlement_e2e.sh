#!/usr/bin/env bash
# E2E — التسوية والمطابقة المالية (المرحلة S1): ختم التأكيد + صف التسوية +
# المزامنة السحبية عبر GET + طابور المراجعة + الاسترداد المُسجَّل — قاعدة نظيفة.
...
echo "== تشغيل الباكند على 8080 (البوابتان + مستخدم المنصة الفوري) =="
...
setsid nohup /tmp/pharmacy-backend > /tmp/backend_settlement_e2e.log 2>&1 &
...
echo "== E2E: Settlement & Reconciliation =="
python3 scripts/settlement_e2e.py
STATUS=$?

pkill -f 'pharmacy-backend$' 2>/dev/null
[ $STATUS -eq 0 ] && echo "SETTLEMENT E2E OK" || echo "SETTLEMENT E2E FAILED"
exit $STATUS
