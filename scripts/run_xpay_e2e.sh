#!/usr/bin/env bash
# E2E — بوابة XPay (المرحلة X): جلسات checkout + ويب هوك موقّع على قاعدة نظيفة.
# كل شيء في نفس استدعاء shell (الخلفيات تُقتل بين الاستدعاءات).
set -uo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."

export PATH="/home/z/.venv/bin:$PATH"
export GOTOOLCHAIN=auto CGO_ENABLED=0 GOFLAGS=-mod=vendor
export DATABASE_URL="postgresql://postgres@127.0.0.1:54329/reports_test?sslmode=disable"

GO_BIN="$(command -v go || true)"
[ -z "$GO_BIN" ] && [ -x /tmp/gotool/bin/go ] && GO_BIN=/tmp/gotool/bin/go
[ -z "$GO_BIN" ] && [ -x /tmp/go/bin/go ] && GO_BIN=/tmp/go/bin/go
[ -z "$GO_BIN" ] && { echo "go not found"; exit 1; }

echo "== PostgreSQL =="
LD_LIBRARY_PATH=/tmp/pg17/lib /tmp/pg17/bin/pg_ctl -D /home/z/pgdata status >/dev/null 2>&1 \
  || LD_LIBRARY_PATH=/tmp/pg17/lib /tmp/pg17/bin/pg_ctl -D /home/z/pgdata -o "-p 54329 -k /tmp" -l /home/z/pgdata/log.txt start >/dev/null 2>&1
sleep 1

echo "== Reset: reports_test نظيفة =="
python3 - <<'PY'
import psycopg2
conn = psycopg2.connect("postgresql://postgres@127.0.0.1:54329/postgres")
conn.autocommit = True
cur = conn.cursor()
cur.execute("SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='reports_test' AND pid <> pg_backend_pid()")
cur.execute("DROP DATABASE IF EXISTS reports_test")
cur.execute("CREATE DATABASE reports_test")
cur.execute("SELECT 1 FROM pg_database WHERE datname='reports_test'")
assert cur.fetchone(), "reports_test missing after create"
print("reports_test recreated (fresh)")
PY

echo "== بناء الباكند =="
(cd backend && "$GO_BIN" build -o /tmp/pharmacy-backend ./cmd/server) || exit 1
pkill -f 'pharmacy-backend$' 2>/dev/null; sleep 0.5

echo "== تشغيل الباكند على 8080 (البوابتان معاً — الأولوية لإكس باي) =="
setsid nohup /tmp/pharmacy-backend > /tmp/backend_xpay_e2e.log 2>&1 &
BACK_PID=$!
export XPAY_SECRET_KEY=sk_test_e2e XPAY_PUBLISHABLE_KEY=pk_test_e2e
export XPAY_WEBHOOK_SECRET=whsec_e2e_secret XPAY_WEBHOOK_TOKEN=xpay_e2e_token
export XPAY_BASE_URL=http://127.0.0.1:9555
export PAYMOB_API_KEY=fake_secret PAYMOB_PUBLIC_KEY=fake_pk
export PAYMOB_CARD_INTEGRATION_ID=123 PAYMOB_HMAC_SECRET=fake_hmac PAYMOB_WEBHOOK_TOKEN=fake_token
export PUBLIC_APP_URL=http://localhost:3000
# إعادة تشغيل مع البيئة (العملية الأولى بدون env)
kill $BACK_PID 2>/dev/null; sleep 0.5
setsid nohup /tmp/pharmacy-backend > /tmp/backend_xpay_e2e.log 2>&1 &
for _ in $(seq 1 60); do
  curl -s -o /dev/null --max-time 2 http://localhost:8080/api/v1/health && break
  sleep 0.5
done
curl -s http://localhost:8080/api/v1/health | head -c 200; echo

echo "== E2E: XPay =="
python3 scripts/xpay_e2e.py
STATUS=$?

pkill -f 'pharmacy-backend$' 2>/dev/null
[ $STATUS -eq 0 ] && echo "XPAY E2E OK" || echo "XPAY E2E FAILED"
exit $STATUS
