...
import '../print/report_pages.dart';
...
/// (التنفيذ المشترك للطباعة والشاشة في print/report_pages.dart)
String _fmtDay(String iso) => reportDayLabel(iso);

/// ساعة 12 بصيغة ص/م — التنفيذ المشترك في print/report_pages.dart
String _fmtClock(String iso, {bool padHour = false}) =>
    reportClockLabel(iso, padHour: padHour);

/// «12 شريط» — التنفيذ المشترك في print/report_pages.dart
String _quantityText(num quantity, String unit) =>
    quantityTextWithUnit(quantity, unit);

// ------------------------------------------------- آلة الطباعة/التصدير المشتركة

/// آلة مشتركة لshاشات التقارير الثلاثة: تبني صفحات A4 خارج حدود الشاشة
/// فتُصمّم وتُرسَم دون أن تُرى، ثم يلتقطها RepaintBoundary ويطبعها عبر
/// نافذة الطباعة في النظام (layoutPdf) أو يشاركها ملف PDF (sharePdf).
mixin _ReportExportMixin<T extends StatefulWidget> on State<T> {
  ...
  Future<void> _runExport(
    ...
    required bool printer,
    required String jobName,
    required List<Widget> Function() pages,
  }) async {
    final i18n = AppI18n.instance;
    ...
    setState(() {
      ...
      _exportKeys
        ..clear()
        ..addAll(List<GlobalKey>.generate(built.length, (_) => GlobalKey()));
    });
    try {
      // انتظار نضوج أول إطار بعد إدراج الصفحات (تصميم + رسم) قبل الالتقاط
      await WidgetsBinding.instance.endOfFrame;
      await Future<void>.delayed(const Duration(milliseconds: 120));
      final ReportExportResult result = printer
          ? await printReportPdf(boundaryKeys: _exportKeys, jobName: jobName)
          : await shareReportPdf(boundaryKeys: _exportKeys, jobName: jobName);
      if (!mounted) return;
      if (!result.ok) {
        await appSnackbar(context,
            i18n.t('reports', printer ? 'print_failed' : 'export_failed'),
            error: true);
      }
    } catch (_) {
      if (mounted) {
  ...
  /// زرّا AppBar: طباعة النظام + تصديr PDF — معطّلان أثناء التجهيز
  List<Widget> exportActions(
    ...
    return <Widget>[
      IconButton(
        tooltip: i18n.t('reports', 'print_report'),
        onPressed: _exporting
            ? null
            : () => _runExport(context,
                printer: true, jobName: jobName, pages: pages),
        icon: _exporting
            ? const SizedBox(
                width: 18, height: 18,
      ...
      IconButton(
        ...
        onPressed: _exporting
            ? null
            : () => _runExport(context,
                printer: false, jobName: jobName, pages: pages),
        icon: const Icon(Icons.picture_as_pdf_outlined),
      ),
    ];
  }

  /// صفحات A4 أثناء التصدير: موضوعة خارج حدود الـStack فتُصمّم وتُرسَم
  /// دون أن تُرى، ويلتقطها RepaintBoundary لصفحات الـPDF.
  Widget exportOverlay() {
    return Positioned(
      left: -20000,
      top: 0,
      ...
      child: Column(
        children: <Widget>[
          for (int i = 0; i < (_exportPages?.length ?? 0); i++)
...
class _SalesReportScreenState extends State<SalesReportScreen>
  ...
  // ------------------------------------------ تصدير/طباعة PDF (المرحلة 4)
  //
  // تصدير وطباعة محليان 100% يعملان بلا إنترنت: البيانات في الذاكرة (من
  // الكاش المشفّر عند الانقطاع)، والصفحات ويدجت Flutter تُلتقط
  // RepaintBoundary ← PDF A4 ← نافذة طباعة النظام أو مشاركة الملف.
  // البنّائات المشتركة للتقارير الثلاثة في print/report_pages.dart.
  ...
  List<Widget> _buildExportPages(SalesReport r, AppState state) {
    ...
    contents.add(Column(
      ...
      children: <Widget>[
        reportHeader(
          ...
          title: i18n.t('reports', 'sales_title'),
          periodText: '${reportDayLabel(_from ?? '')} – ${reportDayLabel(_to ?? '')}',
        ),
        const SizedBox(height: 14),
        reportKpiGrid(<({String label, String value, String? hint})>[
          for (final _KpiItem k in _kpis(r))
            (label: k.label, value: k.value, hint: k.hint),
        ]),
        const SizedBox(height: 16),
    ...
    if (r.topProducts.isNotEmpty) {
      contents.add(Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          reportSectionTitle(i18n.t('reports', 'top_products')),
          const SizedBox(height: 10),
          reportTable(
            header: <String>[
              '#',
            ...
            rows: <List<Widget>>[
              for (int i = 0; i < r.topProducts.length; i++)
                <Widget>[
                  Text('${i + 1}', style: reportCellStyle(dim: true)),
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisSize: MainAxisSize.min,
                    children: <Widget>[
                  ...
                  Text(Fmt.number(r.topProducts[i].qtyBase), style: reportCellStyle()),
                  Text(_money(r.topProducts[i].amount), style: reportCellStyle(w: FontWeight.w800)),
                ],
            ],
          ),
        ],
    ...
    for (final List<SaleSummary> chunk
        in chunkReportRows(_invoices, _exportRowsPerPage)) {
      contents.add(Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          reportSectionTitle(i18n.t('reports', 'period_invoices')),
          const SizedBox(height: 10),
          reportTable(
            header: <String>[
            ...
            rows: <List<Widget>>[
              for (final SaleSummary s in chunk)
                <Widget>[
                  ...
                  Column(
                    ...
                    mainAxisSize: MainAxisSize.min,
                    children: <Widget>[
                      Text(_fmtSaleDate(s.createdAt), style: reportCellStyle(size: 10)),
                      Text(_fmtClock(s.createdAt), style: reportCellStyle(size: 9, dim: true)),
                    ],
                  ),
                  Text(_saleStatusLabel(s.status),
                      style: reportCellStyle(size: 10, w: FontWeight.w700)),
                  Text(Fmt.number(s.productsCount), style: reportCellStyle(size: 10)),
                  Text(Fmt.number(s.totalQuantityBase), style: reportCellStyle(size: 10)),
                  Text(_money(s.totalAmountPiastres),
                      style: reportCellStyle(size: 10, w: FontWeight.w800)),
                  Text(s.returnedAmountPiastres > 0 ? _money(s.returnedAmountPiastres) : '—',
                      style: reportCellStyle(
                          size: 10,
                          color: s.returnedAmountPiastres > 0
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        // المرحلة 4 — طباعة النظام + تصدير PDF محلي 100% (يعمل من الكاش بلا إنترنت)
        actions: r == null
            ? null
            : exportActions(
                context,
                jobName: 'sales-report-${_from ?? ''}_${_to ?? ''}',
                pages: () => _buildExportPages(r, context.read<AppState>()),
              ),
      ),
      body: Stack(
        fit: StackFit.expand,
...
class _InventoryReportScreenState extends State<InventoryReportScreen>
  ...
  /// صفحات A4 لتقرير المخزون — عبر البنّاء المشترك في print/report_pages.dart
  List<Widget> _buildInventoryExportPages() {
    final InventoryReport? r = _report;
    if (r == null) return const <Widget>[];
    final AppState state = context.read<AppState>();
    final String pharmacyName = state.context?.pharmacyName.trim() ?? '';
    return buildInventoryReportPages(
      report: r,
      items: _items,
      pharmacyName: pharmacyName,
      detailsNote:
          _items.length >= 500 ? AppI18n.instance.t('reports', 'first_500') : null,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(i18n.t('reports', 'inventory_title')),
        actions: _report == null
            ? null
            : exportActions(
                context,
                jobName: 'inventory-report-${Fmt.isoDay(DateTime.now())}',
                pages: () => _buildInventoryExportPages(),
              ),
      ),
      body: Stack(
        fit: StackFit.expand,
...
class _MovementsReportScreenState extends State<MovementsReportScreen>
  ...
  /// صفحات A4 لتقرير الحركات — عبر البنّاء المشترك في print/report_pages.dart
  List<Widget> _buildMovementsExportPages() {
    final MovementsReport? r = _report;
    if (r == null) return const <Widget>[];
    final AppState state = context.read<AppState>();
    final String pharmacyName = state.context?.pharmacyName.trim() ?? '';
    return buildMovementsReportPages(
      report: r,
      details: _details,
      detailsTotal: _detailsTotal,
      fromIso: _from,
      toIso: _to,
      pharmacyName: pharmacyName,
      typeLabel: _typeLabel,
      detailsLimitNote: _detailsTotal > _detailsLimit
          ? AppI18n.instance.t('reports', 'details_showing',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(i18n.t('reports', 'movements_title')),
        actions: _report == null
            ? null
            : exportActions(
                context,
                jobName: 'movements-report-${_from ?? ''}_${_to ?? ''}',
                pages: () => _buildMovementsExportPages(),
              ),
      ),
      body: Stack(
        fit: StackFit.expand,
