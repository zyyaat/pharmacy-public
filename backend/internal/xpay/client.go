...
// RetrieveCheckoutSession calls GET {base}/checkout/sessions/:id — the
// pull half of reconciliation. When a webhook is lost or arrives out of
// order, this is how the backend asks XPay directly what actually happened
// to a session (docs.xpay.app → Retrieve a checkout session). The response
// shape is identical to the creation payload, so SessionResponse parses it
// unchanged; the caller decides what (if anything) to transition — this
// method never touches our state.
func (c *Client) RetrieveCheckoutSession(ctx context.Context, sessionID string) (*SessionResponse, error) {
        if c.SecretKey == "" {
                return nil, fmt.Errorf("xpay: missing secret key")
        }
        if sessionID == "" {
                return nil, fmt.Errorf("xpay: missing session id")
        }
        httpReq, err := http.NewRequestWithContext(ctx, http.MethodGet,
                c.BaseURL+"/checkout/sessions/"+sessionID, nil)
        if err != nil {
                return nil, fmt.Errorf("xpay: build retrieve request: %w", err)
        }
        httpReq.Header.Set("Authorization", "Bearer "+c.SecretKey)

        httpResp, err := c.HTTP.Do(httpReq)
        if err != nil {
                return nil, fmt.Errorf("xpay: retrieve call failed: %w", err)
        }
        defer httpResp.Body.Close()
        raw, err := io.ReadAll(io.LimitReader(httpResp.Body, 1<<20))
        if err != nil {
                return nil, fmt.Errorf("xpay: read retrieve response: %w", err)
        }
        var decoded map[string]any
        if err := json.Unmarshal(raw, &decoded); err != nil {
                return nil, fmt.Errorf("xpay: decode retrieve response (http %d): %w", httpResp.StatusCode, err)
        }
        if httpResp.StatusCode == http.StatusNotFound {
                return nil, fmt.Errorf("xpay: session not found: %s", sessionID)
        }
        if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
                return nil, fmt.Errorf("xpay: retrieve rejected (http %d): %.300s", httpResp.StatusCode, string(raw))
        }
        ...
        if resp.ID == "" {
                return nil, fmt.Errorf("xpay: retrieve response missing id: %.200s", string(raw))
        }
        return resp, nil
}
