...
type Config struct {
        ...
        // Debug exposes internal error details in API responses.
        // Enable ONLY while diagnosing deployment problems: APP_DEBUG=true
        Debug bool
}
...
func Load() *Config {
        ...
        log.Printf("[CONFIG] Port: %s, Environment: %s, Debug: %v, CookieSecure: %v, CORS_ORIGINS: %s, DatabaseURL: %s",
                cfg.Port, cfg.Environment, cfg.Debug, cfg.CookieSecure, cfg.CorsOrigins, maskDatabaseURL(cfg.DatabaseURL))
        if cfg.Debug {
                log.Printf("[CONFIG] APP_DEBUG=true: API error responses will include internal details. Disable this after diagnosis!")
        }
