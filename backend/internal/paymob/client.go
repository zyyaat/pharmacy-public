...
//	          Authorization: Token <API key>          (server-only secret)
...
type Client struct {
	APIKey              string // server-only secret
...
func New(apiKey, publicKey, cardIntegrationID, walletIntegrationID, baseURL string) *Client {
	return &Client{
		APIKey:              apiKey,
		PublicKey:           publicKey,
...
func (c *Client) CreateIntention(ctx context.Context, req IntentionRequest) (*IntentionResponse, error) {
	if c.APIKey == "" || c.CardIntegrationID == "" {
		return nil, fmt.Errorf("paymob: missing API key or card integration ID")
	}
