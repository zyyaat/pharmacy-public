...
type posSaleRequest struct {
        ...
        // خصم على مستوى الفاتورة: نسبة مئوية (0-100) أو مبلغ ثابت بالقروش
        Discount *posSaleDiscountRequest `json:"discount"`
        // نقدي (افتراضي) أو آجل لعميل مسجل في نفس الصيدلية
        PaymentType string `json:"payment_type"`
        CustomerID  string `json:"customer_id"`
}

type posSaleDiscountRequest struct {
        Kind  string  `json:"kind"` // "percent" | "amount"
...
func (h *Handler) LookupPOSProduct(c *gin.Context) {
        ...
        err := h.db.QueryRow(c.Request.Context(), `
                SELECT pp.id::text, COALESCE(gp.name::text, ''), COALESCE(gp.generic_name::text, ''),
                       COALESCE(gp.strength::text, ''), COALESCE(gp.barcode::text, ''),
                       COALESCE(pp.packaging_type::text, ''), COALESCE(pp.units_per_box::int8, 1),
                       pp.selling_price::int8, COALESCE(pp.partial_selling_price::int8, 0),
                       ROUND(COALESCE(SUM(ci.quantity), 0))::int8
                FROM pharmacy_products pp
                ...
                GROUP BY pp.id, gp.name, gp.generic_name, gp.strength, gp.barcode, pp.packaging_type,
                         pp.units_per_box, pp.selling_price, pp.partial_selling_price
        `, pharmacyID, barcode).Scan(&id, &name, &genericName, &strength, &productBarcode, &packagingType, &unitsPerBox, &sellingPrice, &partialPrice, &stock)
        ...
        c.JSON(http.StatusOK, gin.H{"data": gin.H{
                "id": id, "name": name, "generic_name": genericName, "strength": strength, "barcode": productBarcode,
                "packaging_type": packagingType, "units_per_box": unitsPerBox,
                "selling_price_piastres": sellingPrice, "partial_selling_price_piastres": partialPrice,
                "stock": stock,
        }})
...
func (h *Handler) CreatePOSSale(c *gin.Context) {
        ...
        // نوع الدفع: نقدي افتراضياً، والآجل يتطلب عميلاً مسجلاً في نفس الصيدلية.
        paymentType := strings.TrimSpace(request.PaymentType)
        if paymentType == "" {
                paymentType = "cash"
        }
        if paymentType != "cash" && paymentType != "credit" {
                c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_payment_type", "message": "نوع الدفع غير معروف"})
                return
        }
        ...
        if paymentType == "credit" && customerID == "" {
                c.JSON(http.StatusBadRequest, gin.H{"error": "customer_required", "message": "البيع الآجل يتطلب اختيار عميل"})
                return
        }
        ...
        // الخصم: يُحسب من الإجمالي ثم يُوزّع على الأسطر بطريقة أكبر باقٍ
        // بحيث يكون مبلغ كل سطر صافياً بعد الخصم — فاسترجاع أي سطر يرد
        // ما حُمّل فعلاً بلا أي معالجة خاصة في تدفق الاسترجاع.
        var subtotal money.Piastres
        for _, lt := range lineTotals {
                subtotal = subtotal.Add(lt)
        }
        discount := money.Piastres(0)
        if request.Discount != nil && request.Discount.Value != 0 {
                switch request.Discount.Kind {
                case "percent":
                        if request.Discount.Value < 0 || request.Discount.Value > 100 {
                                c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_discount", "message": "نسبة الخصم يجب أن تكون بين 0 و 100"})
                                return
                        }
                        discount = money.Piastres(int64(math.Round(float64(subtotal) * request.Discount.Value / 100.0)))
                case "amount":
                        if request.Discount.Value < 0 {
                                c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_discount", "message": "قيمة الخصم لا يمكن أن تكون سالبة"})
                                return
                        }
                        discount = money.Piastres(int64(math.Round(request.Discount.Value)))
                default:
                        c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_discount", "message": "نوع الخصم غير معروف"})
                        return
                }
                if discount > subtotal {
                        c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_discount", "message": "الخصم أكبر من إجمالي الفاتورة"})
                        return
                }
        }
        weights := make([]int64, len(lineTotals))
        for i, lt := range lineTotals {
                weights[i] = int64(lt)
        }
        lineDiscounts := money.Allocate(discount, weights)
        itemAmounts := make([]money.Piastres, len(lineTotals))
        for i := range lineTotals {
                itemAmounts[i] = lineTotals[i] - lineDiscounts[i]
        }

        // Phase 2: persist the invoice.
        var saleID string
        insertErr := tx.QueryRow(c.Request.Context(), `
                INSERT INTO sales (pharmacy_id, branch_id, employee_id, company_user_id, idempotency_key, payment_type, customer_id)
                VALUES ($1, $2, NULLIF($3, '')::uuid, NULLIF($4, '')::uuid, NULLIF($5, ''), $6, NULLIF($7, '')::uuid)
                ON CONFLICT (pharmacy_id, idempotency_key) WHERE idempotency_key IS NOT NULL
                DO NOTHING
                RETURNING id::text
        `, principal.PharmacyID, branchID, employeeID, companyUserID, request.IdempotencyKey, paymentType, customerID).Scan(&saleID)
        ...
        for i, item := range request.Items {
                if err := sellPOSItem(c.Request.Context(), tx, principal, branchID, saleID, item, snapshots[i], itemAmounts[i]); err != nil {
                ...
                total = total.Add(itemAmounts[i])
        }
        // total هنا هو الصافي بعد الخصم (مجموع الأسطر الموزّعة) ويساوي subtotal - discount
        if _, err := tx.Exec(c.Request.Context(), `UPDATE sales SET total_amount = $2, discount_amount = $3 WHERE id = $1`, saleID, total, discount); err != nil {
        ...
        c.JSON(http.StatusCreated, gin.H{"data": gin.H{
                "sale_id": saleID, "total_amount_piastres": total, "discount_amount_piastres": discount, "replayed": false,
        }})
}
