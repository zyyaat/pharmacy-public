...
func (h *Handler) ListPharmacyBranches(c *gin.Context) {
        ...
        //
        // Task 52 — شكوى «بطاقة الفرع مفيش فيها البيانات الحقيقية»: عند التسجيل
        // يُزرع الفرع الرئيسي باسمه وكوده فقط (service.go/bootstrap.go) وتُحفظ
        ...
        rows, err := h.db.Query(c.Request.Context(), `
                ...
                ORDER BY COALESCE(p.default_branch_id = b.id, false) DESC, b.is_active DESC, b.name
                LIMIT 200
        `, pharmacyID)
        if err != nil {
