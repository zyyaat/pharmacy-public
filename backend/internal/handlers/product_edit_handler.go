...
func (h *Handler) GetPharmacyProduct(c *gin.Context) {
	...
	`, idFromParam(c, "id"), pharmacyID).
		Scan(&name, &genericName, &barcode, &packagingType, &unitsPerBox,
	...
	c.JSON(http.StatusOK, gin.H{"data": gin.H{
		...
		"barcode": barcode, "packaging_type": packagingType,
...
func (h *Handler) UpdatePharmacyProduct(c *gin.Context) {
	...
	if _, err := tx.Exec(c.Request.Context(), `
		...
		WHERE id = $1::uuid
	`, globalProductID, request.Name, request.GenericName, request.Barcode); err != nil {
	...
	c.JSON(http.StatusOK, gin.H{"data": gin.H{
		...
		"barcode": request.Barcode, "packaging_type": request.PackagingType,
