...
// Context keys mirroring middleware.ContextRequestID / middleware.ContextDebug.
// Local copies are used (instead of importing the middleware package) to keep
// the auth package free of import-cycle risks.
const (
        requestIDContextKey = "request_id"
        debugContextKey     = "app_debug"
)

// requestIDFrom extracts the correlation ID set by middleware.RequestID().
...
// debugFrom reports whether APP_DEBUG allows exposing internal details.
func debugFrom(c *gin.Context) bool {
        if v, ok := c.Get(debugContextKey); ok {
                if b, ok := v.(bool); ok {
                        return b
                }
        }
...
func (h *Handler) login(c *gin.Context) {
        ...
        if err != nil {
                ...
                case errors.Is(err, ErrAccountLocked):
                        writeErrorDetail(c, http.StatusLocked, "account_locked", "Account temporarily locked", err.Error())
                ...
                case errors.Is(err, ErrInvalidCredentials):
                        ...
                        writeErrorDetail(c, http.StatusUnauthorized, "invalid_credentials", "Invalid email or password", err.Error())
                default:
                        // Unknown failure (usually a database error). Log it and, in
                        // debug mode, surface the real cause to the caller.
                        log.Printf("[AUTH] login errored: email=%s principal_type=%s err=%v request_id=%s",
                                req.Email, req.AccountType, err, requestIDFrom(c))
                        writeErrorDetail(c, http.StatusInternalServerError, "login_failed",
                                "Login could not be completed", err.Error())
                }
                return
        }
...
func writeError(c *gin.Context, status int, code, message string) {
        writeErrorDetail(c, status, code, message, "")
}

// writeErrorDetail emits the structured error envelope used across the auth
// API. Every response carries the correlation request_id (echoed on the
// X-Request-ID header too) so a failing frontend action can be matched with
// the exact backend log line. When APP_DEBUG=true and an internal detail is
// supplied, the REAL reason (e.g. the raw database error) is embedded under
// "debug.internal_reason" — this is what makes deployment problems visible
// without SSH-ing into the server.
func writeErrorDetail(c *gin.Context, status int, code, message, detail string) {
        ...
        if detail != "" {
                log.Printf("[AUTH] %s (status=%d request_id=%s): %s", code, status, requestID, detail)
                if debugFrom(c) {
                        body["debug"] = gin.H{"internal_reason": detail}
                }
        }
        c.JSON(status, body)
}
