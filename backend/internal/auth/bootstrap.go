...
func (s *Service) BootstrapSuperAdmin(
        ...
        // SaaS plans (Task 90): the bootstrap platform company must own an
        // active subscription so the plan gate resolves cleanly (even though
        // super admins bypass it in practice).
        var enterprisePlanID string
        if err := tx.QueryRow(ctx, `
                SELECT id::text FROM plans WHERE slug = 'enterprise' AND deleted_at IS NULL
                ORDER BY sort_order LIMIT 1
        `).Scan(&enterprisePlanID); err != nil {
                return fmt.Errorf("resolve enterprise plan for bootstrap: %w", err)
        }
        if _, err := tx.Exec(ctx, `
                INSERT INTO subscriptions
                    (company_id, plan_id, status, billing_interval,
                     current_period_start, source)
                VALUES ($1, $2, 'active', 'none', NOW(), 'manual')
        `, companyID, enterprisePlanID); err != nil {
                return fmt.Errorf("create bootstrap subscription: %w", err)
        }
