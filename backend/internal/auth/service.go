...
func (s *Service) RegisterCompany(ctx context.Context, companyName, companyEmail, firstName, lastName, email, password string) (*Principal, error) {
        ...
        var trialDays int
        var trialPlanSlug string
        if err := tx.QueryRow(ctx, `
                SELECT COALESCE((setting_value->>'default_trial_days')::int, 30),
                       COALESCE(setting_value->>'default_plan_slug', 'professional')
                FROM platform_settings
                WHERE setting_key = 'trial'
        `).Scan(&trialDays, &trialPlanSlug); err != nil {
        ...
        if err := tx.QueryRow(ctx, `
                ...
                VALUES ($1, $2, 'trial', $3, NOW() + ($4 * INTERVAL '1 day'))
                RETURNING id::text
        `, strings.TrimSpace(companyName), normalizeEmail(companyEmail), trialPlanSlug, trialDays).Scan(&companyID); err != nil {
        ...
        // SaaS plans (Task 90): the trial IS a subscription — full-feature
        // experience on the super-admin-chosen default plan, duration from
        // the same platform_settings key. Enforcement reads this row.
        var trialPlanID string
        if err := tx.QueryRow(ctx, `
                SELECT id::text FROM plans WHERE slug = $1 AND deleted_at IS NULL
                ORDER BY sort_order LIMIT 1
        `, trialPlanSlug).Scan(&trialPlanID); err != nil {
                return nil, fmt.Errorf("resolve trial plan %q: %w", trialPlanSlug, err)
        }
        if _, err := tx.Exec(ctx, `
                INSERT INTO subscriptions
                    (company_id, plan_id, status, billing_interval,
                     trial_ends_at, current_period_start, source)
                VALUES ($1, $2, 'trial', 'none',
                        NOW() + ($3 * INTERVAL '1 day'), NOW(), 'registration')
        `, companyID, trialPlanID, trialDays); err != nil {
