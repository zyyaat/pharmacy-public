...
if (useDevelopmentProxy && !process.env.BACKEND_INTERNAL_URL) {
  console.warn('[pos-app] proxy mode active but BACKEND_INTERNAL_URL is NOT set — rewrites target http://127.0.0.1:8080 (local/Replit only). On Vercel set BACKEND_INTERNAL_URL to the public backend origin (https://..., no /api/v1 suffix) and redeploy, or every /api/v1 request fails.')
}
