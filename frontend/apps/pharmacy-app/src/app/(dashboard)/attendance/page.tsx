    ...
    <div className="mx-auto max-w-[1500px] space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {!loading && !error && items.length > 0 && <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-right text-sm"><thead className="border-b text-xs text-muted-foreground"><tr><th className="p-3">الموظف</th><th className="p-3">الفرع</th><th className="p-3">الدخول</th><th className="p-3">الخروج</th><th className="p-3">الحالة</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-b last:border-0"><td className="p-3 font-semibold">{item.employee_name}</td><td className="p-3">{item.branch_name || '—'}</td><td className="p-3">{new Date(item.clock_in).toLocaleString('ar-EG-u-nu-latn')}</td><td className="p-3">{item.clock_out ? new Date(item.clock_out).toLocaleString('ar-EG-u-nu-latn') : '—'}</td><td className="p-3">{item.status === 'active' ? 'حاضر الآن' : item.status === 'completed' ? 'مكتمل' : item.status}</td></tr>)}</tbody></table></div>}
        </CardContent>
      </Card>
    </div>
    </RequirePermission>
