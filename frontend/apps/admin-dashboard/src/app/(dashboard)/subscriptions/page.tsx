...
// practices: an operator overview (active/trials/expiring/MRR KPIs) and
// the effective (one governing row per company) ledger. Task 15: the
// per-row action buttons moved to the FULL ACCOUNT PAGE
// (/companies/[id]) — inline table controls were error-prone and the
// account page hosts subscription actions, overrides, logs and payments
// together. The table stays read-only with one «إدارة» entry point per
// row plus the «السجل» shortcut into the account's logs tab.

import React, { useCallback, useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";
import {
  Ban, Hourglass, Search, CheckCircle2, Users, TrendingUp, AlertTriangle, History,
  Settings2, UserPlus,
} from "lucide-react";
                    ...
                    <td className="px-4 py-3 text-sm">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="icon" title={t("history_btn")}
                          onClick={() => router.push(`/companies/${row.company.id}?tab=logs`)}>
                          <History className="h-4 w-4" />
                        </Button>
                        {/* Task 15: every other control (assign/change plan,
                            extend, suspend/reactivate, cancel, manual
                            payment, overrides) lives on the full account
                            page — inline row buttons invited misclicks. */}
                        <Button variant="outline" size="sm" title={t("acc_manage")}
