...
export default function PermissionsPage() {
  ...
  useEffect(() => {
    permissionsApi.list()
      ...
      .catch((reason) => setError(reason instanceof Error ? reason.message : t("load_failed")))
      ...
      {!loading && !error && (
        <>
          <div className="grid gap-4 sm:grid-cols-3">
            <Metric icon={<Shield className="h-5 w-5 text-primary" />} value={permissions.length} label={t("metric_permissions")} />
            <Metric icon={<ShieldCheck className="h-5 w-5 text-emerald-600" />} value={roles.length} label={t("metric_roles")} />
            <Metric icon={<Users className="h-5 w-5 text-amber-600" />} value={roles.reduce((sum, item) => sum + item.userCount, 0)} label={t("metric_users")} />
          </div>
          <div className="grid gap-6 lg:grid-cols-3">
            <Card>
              <CardHeader><CardTitle>{t("roles_title")}</CardTitle></CardHeader>
              <CardContent className="space-y-2">
                {roles.map((item) => (
                  <button key={item.id} onClick={() => setSelectedRole(item.id)} className={`w-full rounded-lg border p-4 text-start transition ${role?.id === item.id ? "border-primary bg-primary/5" : "border-border hover:bg-accent/30"}`}>
                    <div className="flex items-center justify-between gap-2"><span className="font-medium">{item.name}</span><Badge variant={item.isSystem ? "secondary" : "outline"}>{t("users_count", { count: item.userCount })}</Badge></div>
            ...
            <Card className="lg:col-span-2">
              <CardHeader>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div><CardTitle>{role?.name || t("permissions_fallback")}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{t("role_permissions_count", { count: role?.permissionKeys.length || 0 })}</p></div>
                  <div className="relative sm:w-72"><Search className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("search_placeholder")} className="h-10 w-full rounded-lg border border-input bg-background ps-10 pe-4 text-sm" /></div>
                </div>
              </CardHeader>
              <CardContent className="max-h-[600px] space-y-5 overflow-y-auto">
                ...
                {filteredPermissions.length === 0 && <p className="py-8 text-center text-muted-foreground">{t("no_matches")}</p>}
              </CardContent>
            </Card>
          </div>
        </>
